var app = angular.module('coeplimApp.reporteGastosCtrl', []);

	app.controller('reporteGastosCtrl', ['$scope','$http', '$routeParams', 'Gastos','Parcelas', function($scope,$http,$routeParams,Gastos,Parcelas){
        ...
        Parcelas.all().then(function(){
  		...
  		Gastos.getGastos2().then(function(){
  			$scope.gastos = Gastos.gastos;
				if ($scope.gastos == undefined || Gastos.gastos.length == 0) {
					window.location = '#/reporteGastos';
					return;
				}
  		});
    ...
    $scope.export = function(filtrados){
        var rows = [];
        rows.push(['Fecha', 'Unidad Productiva', 'Actividad', 'Costo']);

        for(var i = 0; i<filtrados.length; i++){
            rows.push([filtrados[i].gst_fecha, filtrados[i].pcl_alias, filtrados[i].ctl_descripcion, filtrados[i].dt_precio]);
        }
        ...
        var contenido = {
            ...
            content: [
                {text: 'Bitácora de Registro de Gastos \n\n', style: 'header'},
                {
                    style: 'tableExample',
                    table: {
...
app.filter("rango_fechas3", function ($filter) {
    return function (items, fecha_i, fecha_f) {
        ...
        return $filter('filter')(items, function (elem) {
            var fecha = moment(elem['gst_fecha']).format(format);
